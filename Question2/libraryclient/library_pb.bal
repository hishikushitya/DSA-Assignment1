...
// Define a constant containing Protocol Buffer descriptors
...
// Define an isolated gRPC client class for LibraryService
public isolated client class LibraryServiceClient {
    *grpc:AbstractClientEndpoint;

    private final grpc:Client grpcClient;

    // Initialize the gRPC client with a URL and configuration
    public isolated function init(string url, *grpc:ClientConfiguration config) returns grpc:Error? {
        self.grpcClient = check new (url, config);
        check self.grpcClient.initStub(self, LIBRARY_DESC);
    }

    // Define remote functions for gRPC service methods
    ...
    // Implement other remote functions similarly
    // Define a streaming client for creating users
...
// Define a client class for streaming user creation
...
// Define context records to carry context information
public type ContextUserStream record {|
    stream<User, error?> content;
    map<string|string[]> headers;
|};

// Define context response types
...
// Define context user record
public type ContextUser record {|
    User content;
    map<string|string[]> headers;
|};

// Define context BorrowRequest record
public type ContextBorrowRequest record {|
    BorrowRequest content;
    map<string|string[]> headers;
|};

// Define context BookList record
public type ContextBookList record {|
    BookList content;
    map<string|string[]> headers;
|};

// Define context Book record
...
// Define context BookLocation record
public type ContextBookLocation record {|
    BookLocation content;
    map<string|string[]> headers;
|};

// Define context BookISBN record
...
// Define a response type for gRPC methods
@protobuf:Descriptor {value: LIBRARY_DESC}
public type Response record {|
    string message = "";
|};

// Define an empty type for gRPC methods
@protobuf:Descriptor {value: LIBRARY_DESC}
...
// Define a BorrowRequest message type
@protobuf:Descriptor {value: LIBRARY_DESC}
...
// Define a BookList message type
@protobuf:Descriptor {value: LIBRARY_DESC}
public type BookList record {|
    Book[] books = [];
|};

// Define a Book message type
@protobuf:Descriptor {value: LIBRARY_DESC}
...
// Define a BookLocation message type
@protobuf:Descriptor {value: LIBRARY_DESC}
public type BookLocation record {|
    string location = "";
|};

// Define a BookISBN message type
@protobuf:Descriptor {value: LIBRARY_DESC}
