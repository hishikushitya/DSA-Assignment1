...
public client class LibraryServiceBookISBNCaller {
    private grpc:Caller caller;
    ...
    isolated remote function sendBookISBN(BookISBN response) returns grpc:Error? {
        return self.caller->send(response);
    }

    isolated remote function sendContextBookISBN(ContextBookISBN response) returns grpc:Error? {
        return self.caller->send(response);
    }
